...
export const mockApi = {
  ...
  async debitWallet(userId, amount, note) { await wait(); const user = state.users.find(item => item.id === userId); if (!user) throw new Error('User not found'); const value = Number(amount); if (value <= 0 || value > user.balance) throw new Error('Debit exceeds available balance'); user.balance -= value; state.transactions.unshift({ id: `TXN-${Date.now().toString(36).toUpperCase()}`, user: user.name, type: 'Wallet debit', amount: value, status: 'success', channel: 'Admin debit', time: 'Just now', reference: `ADMIN_DEBIT_${Date.now()}` }); state.logs.unshift({ time: new Date().toLocaleTimeString(), level: 'INFO', event: note || 'Wallet debit approved', actor: 'Operations', entity: userId }); return clone(user); },
