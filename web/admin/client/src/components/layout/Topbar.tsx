  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between px-4 md:px-6 bg-white border-b border-slate-200">
      <div className="flex items-center gap-3 w-full max-w-3xl">
        <Button variant="ghost" size="icon" className="md:hidden rounded-md" onClick={onMenuClick}>
        ...
        <div className="relative md:hidden w-full">
          ...
          <Input
            type="search"
            placeholder="Search…"
            className="w-full h-9 rounded-md border-slate-200 pl-9 pr-3"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 md:gap-8">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 mt-2 rounded-md">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem className="px-3 py-2 text-red-600 cursor-pointer" onClick={async () => {
              await logout();
            }}>
              Sign out
            </DropdownMenuItem>
