  ...
  return (
    <div className="space-y-8 pb-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Wallet Management</h2>
          <p className="text-slate-500 font-medium">Approve funding requests and perform manual adjustments.</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="border border-slate-200 shadow-sm bg-white rounded-2xl overflow-hidden">
          <CardHeader className="p-6 pb-2">
            <CardTitle className="text-xs font-bold uppercase tracking-wider text-slate-500">Pending Requests</CardTitle>
          </CardHeader>
          <CardContent className="p-6 pt-2">
            <div className="text-2xl font-bold text-slate-900">{deposits.filter(d => d.status === 'pending').length}</div>
            <p className="text-[11px] text-slate-500 font-medium mt-2">Volume ₦{deposits.filter(d => d.status === 'pending').reduce((s, d) => s + Number(d.amount || 0), 0).toLocaleString()}</p>
          </CardContent>
        </Card>
        <Card className="border border-slate-200 shadow-sm bg-white rounded-2xl overflow-hidden">
          <CardHeader className="p-6 pb-2">
            <CardTitle className="text-xs font-bold uppercase tracking-wider text-slate-500">Processed Today</CardTitle>
          </CardHeader>
          <CardContent className="p-6 pt-2">
            <div className="text-2xl font-bold text-slate-900">{deposits.filter(d => {
            ...
            <p className="text-[11px] text-slate-500 font-medium mt-2">Total ₦{deposits.filter(d => {
              const dd = new Date(d.createdAt ? (d.createdAt._seconds ? d.createdAt._seconds * 1000 : d.createdAt) : Date.now());
              const now = new Date();
              return d.status === 'success' && dd.getDate() === now.getDate() && dd.getMonth() === now.getMonth() && dd.getFullYear() === now.getFullYear();
        ...
        <Card className="border border-slate-200 shadow-sm bg-white rounded-2xl overflow-hidden">
          <CardHeader className="p-6 pb-2">
            <CardTitle className="text-xs font-bold uppercase tracking-wider text-slate-500">Monthly Flow</CardTitle>
          </CardHeader>
          <CardContent className="p-6 pt-2">
            <div className="text-2xl font-bold text-slate-900">₦{deposits.filter(d => {
              const dd = new Date(d.createdAt ? (d.createdAt._seconds ? d.createdAt._seconds * 1000 : d.createdAt) : Date.now());
            ...
            <p className="text-[11px] text-slate-500 font-medium mt-2">Performance delta</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="requests" className="space-y-6">
        <TabsList className="bg-white border border-slate-200 p-1.5 rounded-xl">
          <TabsTrigger value="requests" className="rounded-lg px-4 h-9 text-xs font-bold uppercase tracking-wider data-[state=active]:bg-slate-100 transition-all">Requests</TabsTrigger>
          <TabsTrigger value="adjust" className="rounded-lg px-4 h-9 text-xs font-bold uppercase tracking-wider data-[state=active]:bg-slate-100 transition-all">Adjust</TabsTrigger>
          <TabsTrigger value="logs" className="rounded-lg px-4 h-9 text-xs font-bold uppercase tracking-wider data-[state=active]:bg-slate-100 transition-all">Logs</TabsTrigger>
        </TabsList>

        <TabsContent value="requests">
          <Card className="border border-slate-200 shadow-sm bg-white rounded-2xl overflow-hidden">
            <CardHeader className="p-6 pb-2">
              <CardTitle className="text-xl font-bold text-slate-900">Funding Requests</CardTitle>
              <CardDescription className="text-slate-500">Pending wallet deposits</CardDescription>
            </CardHeader>
                ...
                <TableBody>
                  {deposits.length === 0 ? (
                    <TableRow className="border-0">
                      <TableCell colSpan={6} className="text-center py-10 text-slate-500 text-xs font-bold uppercase tracking-wider">No pending requests</TableCell>
                    </TableRow>
                  ) : (
                    deposits.map((d) => (
                      <TableRow key={d.id} className="border-slate-50 hover:bg-slate-50/50 transition-colors group">
                        ...
                        <TableCell className="text-xs text-slate-500 font-medium">{d.method || "manual"}</TableCell>
                        <TableCell>
                          <Badge className={cn(
                        ...
                        <TableCell className="px-6 text-right space-x-2">
                          {d.status === 'pending' && (
                            <>
                              <Button variant="outline" className="h-9 w-9 rounded-lg border-emerald-200 text-emerald-600 hover:bg-emerald-50" onClick={() => handleApprove(d.id)}>
                                <Check className="h-4 w-4" />
                              </Button>
                              <Button variant="outline" className="h-9 w-9 rounded-lg border-red-200 text-red-600 hover:bg-red-50" onClick={() => handleReject(d.id)}>
                                <X className="h-4 w-4" />
                              </Button>
                            </>
                          )}
