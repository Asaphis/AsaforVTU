...
export const api = {
  ...
  async getSettings() {
    ...
    const manualAccounts = Array.isArray(funding.manual_accounts) && funding.manual_accounts.length ? funding.manual_accounts : (funding.manual_bank_name && funding.manual_account_name && funding.manual_account_number ? [{ id: 'legacy-primary', bank_name: funding.manual_bank_name, account_name: funding.manual_account_name, account_number: funding.manual_account_number, enabled: true }] : []);
    return { airtimeNetworks: airtime, cashbackEnabled: Boolean(cashback.enabled), referralBudget: number(referral.daily_budget), referralCampaignStartAt: referral.campaign_start_at || '', referralCampaignEndAt: referral.campaign_end_at || '', automatedFundingEnabled: funding.automated_enabled !== false, manualBankName: funding.manual_bank_name || '', manualAccountName: funding.manual_account_name || '', manualAccountNumber: funding.manual_account_number || '', manualAccounts, manualWhatsappNumber: funding.manual_whatsapp_number || '', manualFundingInstructions: funding.manual_instructions || '', webhook: '' };
  },
  async updateSettings(patch) {
    return request('/api/admin/settings', { method: 'POST', body: { cashbackEnabled: Boolean(patch.cashbackEnabled), dailyReferralBudget: number(patch.referralBudget), referralCampaignStartAt: patch.referralCampaignStartAt || null, referralCampaignEndAt: patch.referralCampaignEndAt || null, automatedFundingEnabled: patch.automatedFundingEnabled !== false, manualBankName: patch.manualBankName || '', manualAccountName: patch.manualAccountName || '', manualAccountNumber: patch.manualAccountNumber || '', manualAccounts: Array.isArray(patch.manualAccounts) ? patch.manualAccounts : [], manualWhatsappNumber: patch.manualWhatsappNumber || '', manualFundingInstructions: patch.manualFundingInstructions || '', ...(patch.airtimeNetworks ? { airtimeNetworks: patch.airtimeNetworks } : {}) } });
  },
  async listNetworks() {
